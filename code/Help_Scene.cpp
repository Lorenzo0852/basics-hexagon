...
namespace sh
{
        ...
        if (state == READY)                     // Se descartan los eventos cuando la escena está LOADING
        {
            switch (event.id)
            {
                ...
                case ID(touch-moved):
                {
                    ...
                    {
                        options[1].is_pressed = index == option_touched;
                    }
                ...
                case ID(touch-ended):           // El usuario deja de tocar la pantalla
                {
                    ...
                    if (option_at (touch_location) == BUTTON)
                    {
                        director.run_scene (shared_ptr< Scene >(new sh::Menu_Scene));
                    }
        ...
        if (!suspended) if (state == LOADING)
            {
                ...
                if (context)
                {
                    ...
                    atlas.reset (new Atlas("help-scene/help_menu.sprites", context));
    ...
    void Help_Scene::configure_options ()
    {
        // Se asigna un slice del atlas a cada opción del menú según su ID:

        options[BUTTON   ].slice = atlas->get_slice (ID(button)   );
        options[DESC   ].slice = atlas->get_slice (ID(desc)   );
    ...
    int Help_Scene::option_at (const Point2f & point)
    {
        for (int index = 0; index < number_of_options; ++index)
        {
